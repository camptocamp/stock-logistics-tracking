...
class TestStockPickingInternal(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.internal_package_type = cls.env["stock.package.type"].create(
            {"name": "Internal Package Type", "package_carrier_type": "none"}
        )
        ...
        cls.internal_package = cls.env["stock.quant.package"].create(
            {"is_internal": True, "package_type_id": cls.internal_package_type.id}
        )
        cls.external_package = cls.env["stock.quant.package"].create(
            {"is_internal": False, "package_type_id": cls.external_package_type.id}
        )
